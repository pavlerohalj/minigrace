...
JSRUNNERS = grace grace-debug minigrace-js minigrace-inspect
...
js/mgc: minigrace.env $(STUBS:%.grace=j2/%.gct)
	...
	awk 'NR>1;/^\/\/ end of preamble/{exit}' js/minigrace-js >> mgcTemp/minigrace-js-head
	awk 'f;/^\/\/ end of preamble/{f=1}' js/minigrace-js  > mgcTemp/minigrace-js-tail
...
minigrace.env: minigrace $(EXTERNAL_STUBS:%.grace=j2/%.js) $(STUBS:%.grace=j2/%.gct) $(JSRUNNERS:%=j2/%)
...
self.test: minigrace.env $(STUBS:%.grace=j2/%.gct)
	...
	echo '#!'"`which node` --max-old-space-size=2048" > selftest/minigrace-js-head
	awk 'NR>1;/^\/\/ end of preamble/{exit}' js/minigrace-js >> selftest/minigrace-js-head
	awk 'f;/^\/\/ end of preamble/{f=1}' js/minigrace-js  > selftest/minigrace-js-tail
	cat selftest/minigrace-js-head j2/gracelib.js $(MGSOURCEFILES:%.grace=j2/%.js) selftest/minigrace-js-tail > selftest/mgc
	...
	cp js/minigrace-js js/minigrace-inspect js/gracelib.js js/tests/harness-js j2/standardGrace.js j2/collectionsPrelude.js selftest
